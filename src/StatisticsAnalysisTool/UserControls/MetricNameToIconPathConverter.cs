    ...
    public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
    {
        ...
        if (value is string name)
        {
            ...
            try
            {
                ...
                // If caller supplied a concrete filename (e.g. "repair_cross.png") treat it as a direct
                // pack resource and attempt to load it before falling back to metric-name -> resource mapping.
                try
                {
                    if (key.IndexOf('.') >= 0)
                    {
                        var fileName = Path.GetFileName(key);
                        // Try Assets first, then Resources
                        var packAssets = $"pack://application:,,,/StatisticsAnalysisTool;component/Assets/{fileName}";
                        var packResources = $"pack://application:,,,/StatisticsAnalysisTool;component/Resources/{fileName}";
                        try
                        {
                            var bitmap = new BitmapImage(new Uri(packAssets, UriKind.Absolute));
                            bitmap.Freeze();
                            _cache.TryAdd(s, bitmap);
                            return bitmap;
                        }
                        ...
                        try
                        {
                            var bitmap = new BitmapImage(new Uri(packResources, UriKind.Absolute));
                            bitmap.Freeze();
                            _cache.TryAdd(s, bitmap);
                            return bitmap;
                        }
                ...
                // Always use Resources for these icons to avoid missing images
                string file = key.ToLowerInvariant() switch
                {
                    "fame" => "Resources/fame.png",
                    "silver" => "Resources/silver.png",
                    "respec" => "Resources/respec.png",
                    "might" => "Resources/might.png",
                    "favor" => "Resources/favor.png",
                    "faction" => "Resources/brecilien_standing_coin.png",
                    _ => "Assets/empty_icon.png"
                };

                // Special handling: if caller passed ConverterParameter == "DamageNameFallback" and
                // the key lookup results in empty_icon.png (no known mapping), prefer to attempt
                // resolving the value as a weapon unique id or choose a random weapon. This fixes
                // situations where damage preview items pass the player's weapon unique name via
                // the Name binding but the converter used the generic fallback.
                try
                {
                    if (parameter is string paramStr && string.Equals(paramStr, "DamageNameFallback", StringComparison.OrdinalIgnoreCase))
                    {
                        // If caller provided a concrete weapon-like name (starts with T and contains '_'), try to load it
                        if (!isRandomWeaponRequest && s.StartsWith("T", StringComparison.OrdinalIgnoreCase) && s.Contains("_"))
                        {
                            var img = StatisticsAnalysisTool.Common.ImageController.GetItemImage(s, 64, 64, true);
                            if (img != null)
                            {
                                _cache.TryAdd(s, img);
                                return img;
                        ...
                        // As a last resort, prefer random weapon selection instead of generic empty icon
                        var randomUnique = PickRandomWeaponUniqueName();
                        if (!string.IsNullOrEmpty(randomUnique))
                        {
                            var randImg = StatisticsAnalysisTool.Common.ImageController.GetItemImage(randomUnique, 64, 64, true);
                            if (randImg != null)
                            {
                                _cache.TryAdd(randomUnique, randImg);
                                return randImg;
                            }
                        }
                    }
                ...
                // file now contains the full relative path
                string packUri = $"pack://application:,,,/StatisticsAnalysisTool;component/{file}";
                try
                {
