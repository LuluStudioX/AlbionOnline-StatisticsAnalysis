...
public class OverlayServer : IDisposable
{
    ...
    // Shared HttpClient for remote image fetches to avoid socket exhaustion and reduce allocations
    private static readonly System.Net.Http.HttpClient _sharedHttpClient = new System.Net.Http.HttpClient();

    // Expose for other overlay modules to reuse the same HttpClient instance
    internal static System.Net.Http.HttpClient SharedHttpClient => _sharedHttpClient;
    ...
    public void Start()
    {
        ...
        // Launch the server run loop on a thread-pool thread to avoid any chance the
        // EmbedIO RunAsync implementation captures the UI SynchronizationContext and
        // blocks the application's UI thread. Use Unwrap() to get the underlying Task.
        var runTask = Task.Run(async () => await _server.RunAsync().ConfigureAwait(false));
    ...
    private WebServer CreateServer(string url)
    {
        ...
        // Serve favicon.ico (with lightweight timing debug logging for slow requests)
        server.WithModule(new ActionModule("/favicon.ico", HttpVerbs.Any, async ctx =>
        {
            var sw = System.Diagnostics.Stopwatch.StartNew();
            const int debugSlowThresholdMs = 50;
            try
            {
            ...
            {
                sw.Stop();
                if (Serilog.Log.IsEnabled(Serilog.Events.LogEventLevel.Debug) && sw.ElapsedMilliseconds > debugSlowThresholdMs)
                {
                    try { Serilog.Log.Debug("[OverlayServer] Slow favicon request: {path} took {ms}ms", ctx.RequestedPath, sw.ElapsedMilliseconds); } catch { }
                }
            }
        }));
        ...
        if (Directory.Exists(overlayStaticDir))
        {
            ...
            server.WithModule(new ActionModule("/Network/Overlay", HttpVerbs.Any, async ctx =>
            {
                ...
                finally
                {
                    ...
                    if (Serilog.Log.IsEnabled(Serilog.Events.LogEventLevel.Debug) && sw.ElapsedMilliseconds > debugSlowThresholdMs)
                    {
                        try { Serilog.Log.Debug("[OverlayServer] Slow static overlay request: {path} took {ms}ms", ctx.RequestedPath, sw.ElapsedMilliseconds); } catch { }
                    }
                }
            }));
        }

        // Single ActionModule for all other routes (HTML and /img) with timing
        server.WithModule(new ActionModule("/", HttpVerbs.Any, async ctx =>
            {
                ...
                try
                {
                        ...
                        if (found == null)
                        {
                            try
                            {
                                ...
                                if (!string.IsNullOrWhiteSpace(name) && !name.Contains("/") && !name.Contains(".."))
                                {
                                    ...
                                    try
                                    {
                                        var resp = await http.GetAsync(remoteUrl).ConfigureAwait(false);
                                    ...
                                    {
                                        LogOverlayDebug($"[ActionModule] Remote fetch failed (http): {ex.Message}");
                                    }
                                }
                            }
                            catch (Exception ex)
                    ...
                    ctx.Response.StatusCode = 404;
                    await ctx.SendStringAsync("Not Found", "text/html", System.Text.Encoding.UTF8).ConfigureAwait(false);
                }
                finally
                {
                    sw.Stop();
                    if (Serilog.Log.IsEnabled(Serilog.Events.LogEventLevel.Debug) && sw.ElapsedMilliseconds > debugSlowThresholdMs)
                    {
                        try { Serilog.Log.Debug("[OverlayServer] Slow request: {path} took {ms}ms", ctx.RequestedPath, sw.ElapsedMilliseconds); } catch { }
                    }
                }
            }));
    ...
    public static string OverlaySettingsToJson(StatisticsAnalysisTool.Overlay.OverlayOptionsObject s)
    {
        ...
        // Repair visibility flags
        var repairShowToday = s.RepairShowToday.ToString().ToLower();
        var repairShowLast7 = s.RepairShowLast7Days.ToString().ToLower();
        var repairShowLast30 = s.RepairShowLast30Days.ToString().ToLower();
        ...
        return $"{{\"fontSize\":{fontSize},\"iconSize\":{iconSize}," +
            ...
            $"\"username\":\"{username}\",\"hideSelf\":{damageHideSelf},\"damageSettings\":{{\"previewCount\":{damagePreviewCount},\"showDps\":{damageShowDps},\"showHeal\":{damageShowHeal},\"showIcons\":{damageShowIcons}}},\"repairShowToday\":{repairShowToday},\"repairShowLast7Days\":{repairShowLast7},\"repairShowLast30Days\":{repairShowLast30}}}}}";
    }
