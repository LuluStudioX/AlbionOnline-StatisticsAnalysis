...
public class StreamingOverlayViewModel : BaseViewModel
{
                ...
                try
                {
                    ...
                    if (db != null && (db.RepairCostsToday != 0 || db.RepairCostsLast7Days != 0 || db.RepairCostsLast30Days != 0))
                    {
                        list.Add(new StatisticsAnalysisTool.Models.MetricDisplayOption
                        {
                            ...
                            ValueFontSize = _overlayOptions.RepairValueFontSize
                        });
                        list.Add(new StatisticsAnalysisTool.Models.MetricDisplayOption
                        {
                    ...
                    list.Add(new StatisticsAnalysisTool.Models.MetricDisplayOption
                    {
                        ...
                        ValueFontSize = buffer.RepairValueFontSize
                    });
                    list.Add(new StatisticsAnalysisTool.Models.MetricDisplayOption
                    {
                        Name = StreamingOverlayTranslation.REPAIR_COSTS_LAST_7_DAYS,
    ...
    // Ensure edit buffer has event handlers attached so PreviewMetrics is refreshed
    private void EnsureEditBufferInitialized()
    {
        if (_editBuffer != null) return;
        _editBuffer = new Overlay.OverlayOptionsObject();
        // Listen for property changes on the buffer itself (font/icon sizes etc.)
        _editBuffer.PropertyChanged += EditBuffer_PropertyChanged;

        // Listen for collection changes on Metrics and per-item property changes
        try
        {
            var metrics = _editBuffer.Metrics;
            if (metrics != null)
            {
                metrics.CollectionChanged += EditBuffer_Metrics_CollectionChanged;
                foreach (var item in metrics)
                {
                    if (item is System.ComponentModel.INotifyPropertyChanged npc)
                        npc.PropertyChanged += EditBufferMetric_PropertyChanged;
                }
            }
        }
        catch { }
    ...
    private void EditBuffer_Metrics_CollectionChanged(object sender, System.Collections.Specialized.NotifyCollectionChangedEventArgs e)
    {
        // Attach handlers for new items
        if (e.NewItems != null)
        {
            foreach (var obj in e.NewItems)
            {
                if (obj is System.ComponentModel.INotifyPropertyChanged npc)
                    npc.PropertyChanged += EditBufferMetric_PropertyChanged;
            }
        }
        // Detach handlers for removed items
        ...
        // Metrics changed -> refresh preview
        OnPropertyChanged(nameof(PreviewMetrics));
    }

    private void EditBufferMetric_PropertyChanged(object sender, System.ComponentModel.PropertyChangedEventArgs e)
    {
        // Any per-metric property change should refresh the preview
        OnPropertyChanged(nameof(PreviewMetrics));
    }

    private void EditBuffer_PropertyChanged(object sender, System.ComponentModel.PropertyChangedEventArgs e)
    {
        // Buffer-level changes (font sizes, icon sizes, toggles) affect preview
        if (string.IsNullOrEmpty(e?.PropertyName))
        {
            OnPropertyChanged(nameof(PreviewMetrics));
            return;
        }
        // List of properties that influence preview rendering
        var watched = new[] {
            "DashboardTitleFontSize","DashboardTotalFontSize","DashboardPerHourFontSize","DashboardIconSize",
            "DamageTitleFontSize","DamageValueFontSize","DamageDpsFontSize","DamageHealFontSize","DamageIconSize",
            "RepairTitleFontSize","RepairValueFontSize","RepairIconSize","DashboardUsePreviewMetrics"
        };
    ...
    private void ApplySettings(object obj)
    {
        ...
        {
            // Only start the overlay server if the user also has the Streaming Overlay navigation tab active
            if (s.IsStreamingOverlayNaviTabActive)
            {
                try { _overlayOptions.StartOverlayIfEnabled(); } catch { }
            }
            else
            {
                Serilog.Log.Debug("[StreamingOverlayViewModel] Overlay enabled in settings but streaming overlay tab is hidden - skipping StartOverlayIfEnabled");
            }
        }
    }
    ...
    {
        Serilog.Log.Information("[StreamingOverlayViewModel] Constructor invoked");
        ...
        try
        {
            if (StatisticsAnalysisTool.Common.ServiceLocator.IsServiceInDictionary<Overlay.OverlayOptionsObject>())
            {
                ...
                Serilog.Log.Information("[StreamingOverlayViewModel] Resolved OverlayOptionsObject from ServiceLocator");
            ...
            else
            {
                ...
                Serilog.Log.Information("[StreamingOverlayViewModel] No shared OverlayOptionsObject found; using local instance");
            }
        }
        catch
        {
            ...
            _overlayOptions = new Overlay.OverlayOptionsObject();
            Serilog.Log.Warning("[StreamingOverlayViewModel] ServiceLocator threw while resolving OverlayOptionsObject; using local instance");
        }
        Instance = this;
        Serilog.Log.Information("[StreamingOverlayViewModel] Instance assigned: {Type}", this.GetType().FullName);
        ...
        _overlayOptions.DashboardTitleFontSize = settings.OverlayDashboardTitleFontSize;
        _overlayOptions.DashboardTotalFontSize = settings.OverlayDashboardTotalFontSize;
        _overlayOptions.DashboardPerHourFontSize = settings.OverlayDashboardPerHourFontSize;
        _overlayOptions.DashboardAutoHideZeroValues = settings.OverlayDashboardAutoHideZeroValues;
        _overlayOptions.DashboardFontSize = settings.OverlayDashboardFontSize;
        _overlayOptions.DashboardIconSize = settings.OverlayDashboardIconSize;
        _overlayOptions.GatheringFontSize = settings.OverlayGatheringFontSize;
        _overlayOptions.GatheringIconSize = settings.OverlayGatheringIconSize;
        _overlayOptions.DamageFontSize = settings.OverlayDamageFontSize;
        _overlayOptions.DamageIconSize = settings.OverlayDamageIconSize;
        // Only activate overlays if enabled in settings
        _overlayOptions.IsOverlayEnabled = settings.OverlayIsEnabled;
        ...
        if (settings.OverlayIsEnabled && resolvedFromServiceLocator)
        {
            try { _overlayOptions.StartOverlayIfEnabled(); } catch { }
            // Overlay entity tracker disabled - relying on app DamageMeter for authoritative totals
        }
        // Ensure edit buffer exists and is wired so preview updates when the UI is shown
        EnsureEditBufferInitialized();
        // Restore per-metric settings from JSON if available
        if (!string.IsNullOrWhiteSpace(settings.OverlayMetricSettingsJson))
        {
            try
            {
                using (var doc = System.Text.Json.JsonDocument.Parse(settings.OverlayMetricSettingsJson))
                {
